...
async def _handle_plan_from_spec(rest: str, ctx: dict) -> str:
    ...
    _emit_plan_event(
        ctx,
        "plan_generate_complete",
        plan_id=plan.plan_id,
        step_count=len(plan.steps),
        ac_count=len(plan.acceptance_criteria),
    )
...
async def _handle_plan_refine(rest, ctx):
    ...
    if not cli or not cli._last_plan:
        return "\033[2mNo plan to refine. Build one first.\033[0m"
    parent = cli._last_plan
    hp = _archive_plan_for_history(parent)
    if hp is None:
        return "\033[2mFailed to archive current plan.\033[0m"
    ...
    _emit_plan_event(
        ...
        new_revision=new_plan.revision,
    )
...
def _archive_plan_for_history(plan):
    ...
    try:
        ...
        p = d / fn
        p.write_text(plan.to_markdown(), encoding="utf-8")
        return p
    except Exception:
        return None
...
def _safe_parse_refined(parent, refined_md, focus):
    ...
    if refined_md and refined_md.strip():
        return ExecutionPlan(
            ...
            alternatives=list(parent.alternatives),
...
def _handle_plan_comment(rest, ctx):
    cli = ctx.get("cli")
    if not cli or not cli._last_plan:
        return "\033[2mNo plan to comment on.\033[0m"
    plan = cli._last_plan
    parts = rest.split(maxsplit=1) if rest else []
    if len(parts) < 2:
        return "\033[2mUsage: /plan comment <step_id> <text...>\033[0m"
    try:
        sn = int(parts[0])
    except ValueError:
        return "\033[2mFirst argument must be a step number.\033[0m"
...
async def _handle_plan(args: str, ctx: dict) -> str:
    ...
    if first_token_lc == "refine":
        return await _handle_plan_refine(rest, ctx)

    if first_token_lc == "comment":
        return _handle_plan_comment(rest, ctx)
